            ...
            <div class="col-md-12">
                ...
                @if($aa->actividad->tipo=='ACTIVIDAD-VACIA')
                    ...
                    @if(count($aa->resultadoactividads)>0)
                        <h3 style="text-align: center; padding: 20px; margin-top: 20px;"><b>EVIDENCIA DE LA
                                ACTIVIDAD</b></h3>
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover">
                                <thead>
                    ...
                    @else
                        <h3 style="text-align: center; padding: 20px; margin-top: 20px;"><b>SOLICITAR CALIFICACIÓN DE LA
                                ACTIVIDAD (Si no realiza este paso, no tendrá calificación para la actividad)</b></h3>
                        <form class="form" role='form' method="POST"
                              action="{{route('realizaractividad.pedircalificacion')}}">
                            @csrf
                            ...
                            <div class="col-md-12" style="margin-top: 20px !important">
                                <div class="form-group">
                                    <button class="btn btn-success icon-btn pull-left" type="submit"><i
                                            class="fa fa-fw fa-lg fa-save"></i>PEDIR CALIFICACIÓN
                                    </button>
                                </div>
                            </div>
                        </form>
                ...
                @if($aa->actividad->tipo=='EXAMEN')
                    ...
                    @else
                        <form class="form" role='form' method="POST"
                              action="{{route('realizaractividad.guardarexamen')}}">
                            @csrf
                            <input type="hidden" name="gmd_id" value="{{$gmd->id}}"/>
                            <input type="hidden" name="periodoacademico_id"
                                   value="{{$gmd->gradomateria->periodoacademico_id}}"/>
                                    ...
                                    <ol>
                                        @foreach($aa->actividad->actividadpreguntas as $py)
                                            <li style="background-color: #eee; border: 1px solid; border-radius: 5px; padding: 10px; margin-top: 10px;">
                                                ...
                                                @else
                                                    <textarea id='respuesta_{{$py->pregunta_id}}'
                                                              name='respuesta_{{$py->pregunta->id}}[]' rows='5'
                            ...
                            <div class="col-md-12" style="margin-top: 20px !important">
                                <div class="form-group">
                                    <button class="btn btn-success icon-btn pull-left" type="submit"><i
                                            class="fa fa-fw fa-lg fa-save"></i>Guardar Respuestas
                                    </button>
                                </div>
                            </div>
                        </form>
                ...
                @if($aa->actividad->tipo=='ACTIVIDAD-RECURSO' or $aa->actividad->tipo=='ACTIVIDAD-ESCRITA')
                    <div class="col-md-12">
                        ...
                        @endif
                        <h3 style="text-align: center; padding: 20px; margin-top: 20px;"><b>CARGAR EVIDENCIA DE LA
                                ACTIVIDAD</b></h3>
                        <h4>Mientras la actividad se encuentre habilitada usted puede corregir el documento y subirlo de
                            nuevo cuantas veces quiera. Tenga en cuenta que si la evidencia que subió ya tiene
                            calificación dicha nota será borrada cuando suba un nuevo documento.</h4>
                        <form class="form" enctype="multipart/form-data" role='form' method="POST"
                              action="{{route('realizaractividad.subirresultado')}}">
                            @csrf
                            <input type="hidden" name="gmd_id" value="{{$gmd->id}}"/>
                            <input type="hidden" name="periodoacademico_id"
                                   value="{{$gmd->gradomateria->periodoacademico_id}}"/>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>De clic aquí para seleccionar el documento evidencia de la solución de la
                                        actividad</label>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Esta funcionalidad permite la realización de actividades y simulacros EBEDUC para una materia,
