 ...
 <ol class="breadcrumb">
     ...
     <li class="active"><a> Banco de Preguntas</a></li>
 </ol>
 @endsection
 @section('content')
 <div class="box">
     <div class="box-header with-border">
         ...
         <div class="box-tools pull-right">
             ...
             <a href="{{route('preguntas.crear',$gmd->id)}}" class="btn btn-box-tool" data-toggle="tooltip" data-original-title="Agregar Pregunta">
     ...
     <div class="box-body">
         ...
         <div class="col-md-12">
             <h3>Listado de Preguntas(solo quién crea la pregunta puede editarla o gestionarla)</h3>
             <div class="table-responsive">
                 <table id="tb" class="table table-bordered table-striped table-hover">
                     ...
                     <tbody>
                         ...
                         <tr>
                             <td>{!! str_limit($a->pregunta, $limit = 50, $end = '...') !!}</td>
                             <td>{{$a->puntos}}</td>
                             <td>{{$a->user->nombres." ".$a->user->apellidos}}</td>
                             <td>{{$a->resp}}</td>
                             <td>
                                 <a href="{{route('preguntas.show',[$gmd->id,$a->id])}}" style='margin-left: 10px; cursor: pointer; color:green;' data-toggle='tooltip' title='Ver'><i class='fa fa-eye'></i></a>
                                 @if(Auth::user()->id==$a->user_id)
                                 <a href="{{route('preguntas.edit',[$gmd->id,$a->id])}}" style='margin-left: 10px; cursor: pointer;' data-toggle='tooltip' title='Editar'><i class='fa fa-edit'></i></a>
                                 @if($a->tipo=='SELECCION MULTIPLE')
                                 <a href="{{route('preguntas.continuar',[$gmd->id,$a->id])}}" style='margin-left: 10px; cursor: pointer; color:orangered;' data-toggle='tooltip' title='Continuar'><i class='fa fa-arrow-right'></i></a>
                                 @endif
                                 @endif
                             </td>
                         </tr>
