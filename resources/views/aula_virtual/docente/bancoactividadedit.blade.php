...
<div class="box">
    ...
    <div class="box-body">
        ...
        <div class="col-md-12" style="display: flex; justify-content: center;">
            <div class="col-md-10" style="border: 1px solid; margin: 10px;">
                ...
                <form class="form" role='form' method="POST" action="{{route('actividad.update')}}">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>Unidad</label>
                            <select class="form-control" name="ctunidad_id" id="ctunidad_id" onchange="getLogros()" required>
                                <option value="">--Seleccione una opción--</option>
                                @foreach($unidades as $key=>$value)
                                @if($a->ctunidadtema->ctunidad_id == $key)
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>Temas</label>
                            <select class="form-control" name="ctunidadtema_id" id="ctunidadtema_id" required="required" disabled>
                                <option selected value="{{$a->ctunidadtema_id}}">{{$a->ctunidadtema->titulo}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <label>Logros</label>
                            <select class="form-control select2" multiple="multiple" style="width: 100%;" name="logros[]" id="logros">
                                <option value="0">-- Seleccione una opción --</option>
                                @foreach($logros as $key=>$value)
                                <?php
                                $existe = false;
                                ?>
                                @foreach($a->ctundestapracts as $m)
                                @if($m->ctunidadestandaraprendizaje_id == $value->id)
                                <?php
                                $existe = true;
                                ?>
                                @endif
                                @endforeach
                                @if($existe)
                                <option value="{{$value->id}}" selected>{{$value->logro}}</option>
                                @else
                                <option value="{{$value->id}}">{{$value->logro}}</option>
                                @endif
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <button class="btn btn-success icon-btn btn-block" type="submit"><i class="fa fa-fw fa-lg fa-save"></i>Guardar
                            </button>
                        </div>
                    </div>
                </form>
            ...
            <div class="col-md-5" style="border: 1px solid; margin: 10px;">
                ...
                <a href="{{asset('documentos/aulavirtual/'.$a->recurso)}}" target="_blank">Ver el Documento
                    Actual</a>
