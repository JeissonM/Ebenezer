...
@section('breadcrumb')
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{route('calificaciondocente.listarestudiantes',[$gmd->id,$a->id])}}"><i class="fa fa-list"></i>
                Listar Estudiantes</a></li>
    ...
    <div class="box">
        ...
        <div class="box-body">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tr>
                            ...
                            <td>{{$a->actividad->ctunidadtema->ctunidad->nombre}}</td>
                            <td>{{$a->actividad->ctunidadtema->titulo}}</td>
            ...
            <div class="col-md-12">
                ...
                @if($cal!=null)
                    <h4 class="text-center text-danger">El estudiante presentó la actividad y posee calificación parcial
                        o completa, puede terminar de calificar la actividad o modificar la calificación que ya
                        tiene.</h4>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover">
                                ...
                                <tr>
                                    ...
                                    <td><a target="_blank"
                                           href="{{asset('documentos/aulavirtual/'.$cal->recurso)}}">{{$cal->recurso}}</a>
                                    </td>
                                </tr>
                                </tbody>
                            </table>
                ...
                <!-- EDITAR -->
                    ...
                    <!-- OTRAS ACTIVIDADES -->
                        <form class="form" role='form' method="POST"
                              action="{{route('calificaciondocente.calificaractividad')}}">
                            @csrf
                            <input type="hidden" name="gmd_id" value="{{$gmd->id}}"/>
                            <input type="hidden" name="periodoacademico_id"
                                   value="{{$gmd->gradomateria->periodoacademico_id}}"/>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="text" required="required"
                                           value="{{$cal->anotaciones_docente}}" name="anotaciones_docente">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="text" required="required"
                                           value="{{$cal->calificacion}}" name="calificacion">
                                </div>
                            </div>
                    ...
                    <!-- EXAMEN O EBEDUC -->
                        ...
                        <!-- HAY EXAMEN REALIZADO, PUEDE CALIFICAR POR SISTEMA -->
                            <form class="form" role='form' method="POST"
                                  action="{{route('calificaciondocente.observaciones')}}">
                                @csrf
                                <input type="hidden" name="gmd_id" value="{{$gmd->id}}"/>
                                <input type="hidden" name="periodoacademico_id"
                                       value="{{$gmd->gradomateria->periodoacademico_id}}"/>
                                <input type="hidden" name="asignaractividad_id" value="{{$a->id}}"/>
                                <input type="hidden" name="grupo_id" value="{{$gmd->grupo_id}}"/>
                                <input type="hidden" name="evaluacionacademica_id"
                                       value="{{$a->evaluacionacademica_id}}"/>
                                <input type="hidden" name="estudiante_id" value="{{$est->id}}"/>
                                <input type="hidden" name="ebeduc" value="{{$a->ebeduc}}"/>
                                <input type="hidden" name="peso" value="{{$a->peso}}"/>
                                <input type="hidden" name="tipo" value="{{$a->actividad->tipo}}"/>
                                        ...
                                        @foreach($a->actividad->actividadpreguntas as $py)
                                            <li style="background-color: #eee; border: 1px solid; border-radius: 5px; padding: 10px; margin-top: 10px;">
                                                            ...
                                                            <div>
                                                                <div class="radio">
                                                                    <label style="color: #000;">
                                                                        <input type="radio"
                                                                               name="respuesta_{{$py->pregunta->id}}[]"
                                                                               value="{{$r->id}}"><span
                                                ...
                                                @else
                                                    ...
                                                    <form class="form" role='form' method="POST"
                                                          action="{{route('calificaciondocente.solocalificacion')}}">
                                                        @csrf
                                                        <input type="hidden" name="gmd_id" value="{{$gmd->id}}"/>
                                                        <input type="hidden" name="asignaractividad_id"
                                                               value="{{$a->id}}"/>
                                                        <input type="hidden" name="estudiante_id" value="{{$est->id}}"/>
                                                        <input type="hidden" name="resactividadresp_id"
                                                               value="{{$resp->id}}"/>
                                                        <div class="form-group">
                                                            <label style="color: #000;">Calificación entre 0
                                                                y {{$py->pregunta->puntos}} (Puntos que usted cree que
                                                                merece la respuesta del estudiante)</label>
                                                            <input class="form-control" type="text" required="required"
                                                                   value="{{$resp->puntos_obtenidos}}"
                                                                   name="puntos_obtenidos">
                                                            <button type="submit" class="btn btn-success"><i
                                                                    class="fa fa-save"></i> GUARDAR CALIFICACIÓN
                                                            </button>
                                                        </div>
                                                    </form>
                                                @endif
                ...
                @else
                <!-- CREAR NUEVA -->
                    <h4 class="text-center text-danger">El estudiante no presentó la actividad, puede asignar una
                        calificación usted mismo, sino lo hace, el sistema calificará en 0 la actividad.</h4>
                    <form class="form" role='form' method="POST"
                          action="{{route('calificaciondocente.calificarcero')}}">
                        @csrf
                        <input type="hidden" name="gmd_id" value="{{$gmd->id}}"/>
                        <input type="hidden" name="periodoacademico_id"
                               value="{{$gmd->gradomateria->periodoacademico_id}}"/>
