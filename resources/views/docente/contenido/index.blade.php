        ...
        $.ajax({
            ...
            if (msg !== "null") {
                ...
                var html = "<table id='tbmatriculados' class='table table-bordered table-striped table-hover'>" +
                    "<thead><tr class='bg-purple'><th>GRADO</th><th>CÓDIGO MATERIA</th><th>MATERIA</th><th>GESTIONAR CONTENIDO</th></tr></thead><tbody>";
                $.each(m, function(index, item) {
                    html = html + "<tr><td>" + item.grado + "</td><td>" + item.codigo + "</td><td>" + item.materia + "</td><td>" +
                        "<a href='" + url + "docente/cargaacademica/docente/contenido/" + item.grado_id + "/" + item.materia_id + "/gestionar' data-toggle='toltip' title='Gestionar Contenido Temático de la Materia' style='margin-left: 10px; color: green;'><i class='fa fa-arrow-right'></i></a></td></tr>";
                });
